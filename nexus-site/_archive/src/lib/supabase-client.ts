...
const getSupabaseConfig = () => {
  console.log('[Supabase] Checking configuration sources...');

  // Check window config first (set by config.js)
  if ((window as any).SUPABASE_URL && (window as any).SUPABASE_ANON_KEY) {
    console.log('[Supabase] Using config from window (config.js)');
    return {
  ...
  if (url && key) {
    console.log('[Supabase] Using config from environment variables');
  } else {
    console.error('[Supabase] No configuration found in window or env vars');
    console.log('[Supabase] window.SUPABASE_URL:', (window as any).SUPABASE_URL);
    console.log('[Supabase] window.SUPABASE_ANON_KEY:', (window as any).SUPABASE_ANON_KEY ? '***' : undefined);
  }
...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('[Supabase] Credentials not found. Team members will not load.');
} else {
...
export async function uploadMemberPhoto(file: File, memberId: string): Promise<string> {
  ...
  if (error) {
    ...
    if (error.message.includes('already exists')) {
      const newFileName = `${memberId}/${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${fileExt}`;
